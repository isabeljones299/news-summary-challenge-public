...
Aims
- To create a React application which allowed a user to see news articles from their phone or laptop which update reqularly from the Guardian website.
- To use static data then a json server to test functionality before using the Guardian API key.
- To consider security when using a personal API key

Improvements
- To handle the error correctly from my try catch block so that it can be rendered instead of the newspage
- To format paragraphs into my article summaries
- To remove/disable clicking of the homepage button from the header when viewing the homepage
